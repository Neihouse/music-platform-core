...
/**
 * StyledTitle - A secure component that extends Mantine's Title with Google Fonts
 * 
 * Features:
 * - Dynamically loads Google Fonts securely (no API key exposure)
      ...
      try {
        ...
        if (isMounted) {
          if (result.success) {
            ...
            setFontError(true);
            setFontLoaded(false);
            console.warn(`Failed to load font "${fontName}":`, result.error);
          }
        }
      } catch (error) {
